...
const colorReg = new RegExp(colorPattern, 'g');
...
class ConsoleGrid {
    ...
    initGridColumns() {
        this.gridColumns = this.columns.map((column, i) => {
            ...
            column.cg_index = i;
            column.cg_key = `cg_${i}_${column.id}`;
            // align
    ...
    initGridHeaders() {
        ...
        this.gridColumns.forEach((column) => {
            ...
            if (lines.length < this.maxHeaderLines) {
                column.cg_lines = new Array(this.maxHeaderLines - lines.length).fill('').concat(lines);
            }
        });
    ...
    getTruncatedCharByWidth(str, w) {
        ...
        const ellipsis = '...';
        const max = w - this.getCharLength(ellipsis);
        const ls = [];
        let len = 0;
        let overflow = false;
        list.forEach((item) => {
            ...
            const valueWidth = this.getCharLength(value);
            if (len + valueWidth < max) {
                ls.push(value);
                len += valueWidth;
                return;
            }

            // truncate character by character for accurate width
            for (const c of value) {
                const cw = this.getCharLength(c);
                if (len + cw > max) {
                    break;
                }
                ls.push(c);
